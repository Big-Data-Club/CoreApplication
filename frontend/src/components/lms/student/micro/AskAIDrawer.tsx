  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside
        ...
        className="absolute right-0 top-0 h-full w-full max-w-md bg-white border-l border-slate-200 shadow-lg flex flex-col"
      >
        <header className="flex items-start justify-between px-5 py-4 border-b border-slate-200">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">{labels.subtitle}</p>
            <p className="text-xs text-slate-400 mt-0.5 truncate max-w-[18rem]">
              {ctx.lessonTitle}
            </p>
          </div>
          <button
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4">
          {messages.length === 0 && !isStreaming ? (
            ...
            <ul className="flex flex-col gap-4">
              ...
              {isThinking && (
                <li className="text-xs text-slate-500">{labels.thinking}</li>
              )}
            </ul>
          )}
        </div>
        ...
        <footer className="border-t border-slate-200 px-5 py-3">
          <div className="flex items-end gap-2">
            <textarea
              ...
              disabled={!userId}
              className="flex-1 resize-none border border-slate-300 rounded-sm bg-white text-sm px-3 py-2 focus:outline-none focus:border-slate-500"
            />
            <button
              type="button"
              onClick={handleSend}
              disabled={!input.trim() || isStreaming || !userId}
              className="px-3 py-2 text-xs font-medium border border-slate-900 bg-slate-900 text-white rounded-sm disabled:bg-slate-400 disabled:border-slate-400"
            >
              {labels.send}
            </button>
          </div>
