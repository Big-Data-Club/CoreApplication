...
async def process_ai_command(payload: dict):
    ...
    try:
        ...
        if command_type == "GENERATE_QUIZ":
            ...
            preview_job_id = job_payload.get("job_id")
            video_id = job_payload.get("youtube_video_id")

            if preview_job_id:
                filepath = f"/tmp/video_jobs/{preview_job_id}/final.mp4"
                if not os.path.exists(filepath):
                    raise FileNotFoundError("Video preview has expired (10-minute limit reached) or does not exist. Please regenerate the video.")
                
                title = job_payload.get("title", "Course Overview")
                description = job_payload.get("description", "Overview generated by AI Course Overview Video Pipeline.")
                
                logger.info(f"Publishing preview video for job {preview_job_id} to YouTube...")
                ...
                # Cleanup local files immediately upon successful upload
                job_dir = f"/tmp/video_jobs/{preview_job_id}"
                if os.path.exists(job_dir):
                    try:
                        shutil.rmtree(job_dir)
                        logger.info(f"Cleaned up local preview folder for job {preview_job_id} after publish")
                ...
                raise ValueError("Missing 'job_id' (for preview upload) or 'youtube_video_id' (for privacy transition) in PUBLISH_VIDEO command payload.")
