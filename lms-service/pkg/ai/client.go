...
func (c *Client) LinkIsolatedNodes(ctx context.Context, courseID int64) (map[string]interface{}, error) {
	var resp map[string]interface{}
	path := fmt.Sprintf("/ai/knowledge-graph/%d/link-isolated", courseID)
	if err := c.post(ctx, path, nil, &resp); err != nil {
		return nil, fmt.Errorf("ai.LinkIsolatedNodes: %w", err)
	}
	return resp, nil
}
...
type GraphEdgeRequest struct {
	...
	RelationType  string  `json:"relation_type"` // prerequisite|extends|related|equivalent|contrasts_with
	Strength      float64 `json:"strength"`
	Reason        string  `json:"reason,omitempty"`
	Bidirectional bool    `json:"bidirectional"`
}

// GraphEdgeResponse is returned after a successful upsert.
...
// UpsertGraphEdge creates a new edge or updates the relation_type/strength of an
// existing edge. Idempotent: same (source, target, relation_type) → UPDATE.
func (c *Client) UpsertGraphEdge(ctx context.Context, req GraphEdgeRequest) (*GraphEdgeResponse, error) {
	var resp GraphEdgeResponse
	if err := c.post(ctx, "/ai/knowledge-graph/edge", req, &resp); err != nil {
		return nil, fmt.Errorf("ai.UpsertGraphEdge: %w", err)
	}
	return &resp, nil
}

// DeleteGraphEdge removes a specific edge (or all edges between a pair if
// RelationType is empty) from PG and Neo4j.
func (c *Client) DeleteGraphEdge(ctx context.Context, sourceID, targetID int64, relationType string) error {
	body := map[string]interface{}{
